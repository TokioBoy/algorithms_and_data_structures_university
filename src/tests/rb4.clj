...
(defn insert-fixup [tree node]
  (let [parent (get-in node [:right])
        grandparent (get-in node [:right :right])
        uncle (get-in node [:right :left])]
    (cond
      (red? parent)
      (if (red? uncle)
        (let [p (assoc parent :color :black)
              u (assoc uncle :color :black)
              gp (assoc grandparent :color :red)
              new-tree (assoc-in tree [:right] gp)]
          (insert-fixup new-tree (assoc node :right new-tree)))
        (if (= node (get-in parent [:right :left]))
          (let [new-tree (assoc-in tree [:right] (right-rotate parent))]
            (insert-fixup new-tree (assoc node :right new-tree)))
          (assoc-in tree [:right] (left-rotate grandparent)))))
    :else
    tree))
